...
class RecuperarController extends \HXPHP\System\Controller
{
	public function __construct($configs)
	{
		...
		$this->view->setTitle("Alteração de Senha");
	...
	public function solicitarAction()
	{
		...
		if(!is_null($email) && $email !== false)
		{
			...
			else
			{
				...
				$messages = $this->messages->messages->getByCode('link-enviado', array(
					'message' => array(
						$validar->user->name,
				...
				$envio_do_email = $this->email->send(
					$validar->user->email,
					$messages['subject'],
					$messages['message'],
					array(
				...
				{
					$messages = $this->messages->messages->getByCode('email-nao-enviado');
				}
